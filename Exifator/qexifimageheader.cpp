...
/*!
    ...
    */
QString QExifValue::toString(QSysInfo::Endian byteOrder) const
{
    switch( d->type )
    {
    case Ascii:
        ...
        case UnicodeEncoding:
        {
            ...
            if( codec ) {
                if ( byteOrder == QSysInfo::BigEndian) {
                    string = QExifImageHeader::changeByteOrder(string);
                }
                QString text = codec->toUnicode( string );
                return text;
            }
        }
        case UndefinedEncoding:
...
/*!
    Constructs a new EXIF image data editor and reads the exif meta-data from a QByteArray.
    */
QExifImageHeader::QExifImageHeader(QByteArray *exifData,quint8 exifId)
    : d(new QExifImageHeaderPrivate)
{
    d->byteOrder = QSysInfo::ByteOrder;
    d->size = -1;
    exifHeaderId = exifId;
    loadFromData(exifData);
}
...
bool QExifImageHeader::modified(void){
    return (readOk && (d->size == -1));
}

quint8 QExifImageHeader::headerId(void){
    return exifHeaderId;
}

QByteArray QExifImageHeader::changeByteOrder(QByteArray text){
...
/*!
    Reads meta-data from a EXIF or META section of an JPEG image.
    Returns true if the data was successfully parsed and false otherwise.
    */
bool QExifImageHeader::loadFromData(QByteArray *exifData) {
    QBuffer buffer(exifData);
    readOk = buffer.open(QIODevice::ReadOnly) && read(&buffer);
    return readOk;
}
